...
#######################################
A set of System Tests is provided in the `tests` directory with a PowerShell test script in `test\script` named `run_system_tests.ps1`.
Follow the instructions below to use the test script. 
  ...
  3. Ensure that the test cases are written in the write format. (Skip this step if you are using the provided test cases)
    - The `analysis.xsl` XML sytlesheet should be present in `test\script`
